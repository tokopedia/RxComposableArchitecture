...
public enum ReducerBuilder<State, Action> {
    ...
    where R.State == State, R.Action == Action {
        ...
        @_disfavoredOverload
        @inlinable
        public static func buildFinalResult<R: ReducerProtocol>(_ reducer: R) -> Reduce<
            State, Action
        >
        where R.State == State, R.Action == Action {
            Reduce(reducer)
    ...
    where R0.State == R1.State, R0.Action == R1.Action {
        ...
        @inlinable
        public func reduce(into state: inout R0.State, action: R0.Action) -> Effect<
            R0.Action
        > {
        ...
        where R0.State == State, R0.Action == Action {
            _Sequence(
                _Sequence(
                    _Sequence(
                        _Sequence(
                            _Sequence(_Sequence(_Sequence(_Sequence(r0, r1), r2), r3), r4), r5), r6
                    ),
                    r7
                ),
                r8
        ...
        >(
            ...
            _Sequence<
                _Sequence<
                    _Sequence<
                        _Sequence<
                            _Sequence<_Sequence<_Sequence<_Sequence<R0, R1>, R2>, R3>, R4>, R5
                        >, R6
                    >,
                    R7
                >,
